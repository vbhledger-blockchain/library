...
import (
	...
	"github.com/vbhledger-blockchain/library/stdocsp"
	"io"
	"io/ioutil"
	"net/http"
	neturl "net/url"
...
var ocspOpts = stdocsp.RequestOptions{
	Hash: crypto.SHA1,
}
...
func certIsRevokedOCSP(leaf *x509.Certificate, strict bool) (revoked, ok bool, e error) {
	...
	ocspRequest, err := stdocsp.CreateRequest(leaf, issuer, &ocspOpts)
	if err != nil {
...
func sendOCSPRequest(server string, req []byte, leaf, issuer *x509.Certificate) (*stdocsp.Response, error) {
	...
	switch {
	case bytes.Equal(body, stdocsp.UnauthorizedErrorResponse):
		return nil, errors.New("OSCP unauthorized")
	case bytes.Equal(body, stdocsp.MalformedRequestErrorResponse):
		return nil, errors.New("OSCP malformed")
	case bytes.Equal(body, stdocsp.InternalErrorErrorResponse):
		return nil, errors.New("OSCP internal error")
	case bytes.Equal(body, stdocsp.TryLaterErrorResponse):
		return nil, errors.New("OSCP try later")
	case bytes.Equal(body, stdocsp.SigRequredErrorResponse):
		return nil, errors.New("OSCP signature required")
	}

	return stdocsp.ParseResponseForCert(body, leaf, issuer)
}
