...
var (
	...
	oidSM2 = asn1.ObjectIdentifier{1, 2, 156, 10197, 2, 1}
)
...
func ParseSm2PublicKey(der []byte) (*sm2.PublicKey, error) {
	...
	if !reflect.DeepEqual(pubkey.Algo.Algorithm, oidSM2) {
		return nil, errors.New("x509ssss: not sm2 elliptic curve")
	}
	curve := sm2.P256Sm2()
...
func ParsePKCS8UnecryptedPrivateKey(der []byte) (*sm2.PrivateKey, error) {
	...
	if !reflect.DeepEqual(privKey.Algo.Algorithm, oidSM2) {
		return nil, errors.New("x509: ccc  not sm2 elliptic curve")
	}
...
func ParsePKCS8PrivateKey(der, pwd []byte) (interface{}, error) {
	if pwd == nil {
		return RawParsePKCS8PrivateKey(der)
	}
...
func ReadPrivateKeyFromMem(data []byte, pwd []byte) (interface{}, error) {
	var block *pem.Block
...
func ReadPrivateKeyFromPem(FileName string, pwd []byte) (interface{}, error) {
...
// 原生解密
func RawParsePKCS8PrivateKey(der []byte) (key interface{}, err error) {
	var privKey pkcs8
	if _, err := asn1.Unmarshal(der, &privKey); err != nil {
		return nil, err
	}
...
func MarshalPKCS8PrivateKey(key interface{}) ([]byte, error) {
	...
	case *sm2.PrivateKey:
		sm2prvikey, ok := key.(*sm2.PrivateKey)
		if !ok{
			return nil, fmt.Errorf("x509:not sm2prvikey  PKCS#8: %T", key)
		}
		return  MarshalSm2UnecryptedPrivateKey(sm2prvikey)
	default:
