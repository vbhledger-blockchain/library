...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	if c.handshakes == 0 {
		...
		if !bytes.Equal(c.peerCertificates[0].Raw, certMsg.certificates[0]) {
			...
			return errors.New("tls: server's identity changed during renegotiation")  // 服务器的身份在重新协商期间发生了变化
		}
	}

	msg, err = c.readHandshake()  // 读取握手
	if err != nil {
		return err
	}
...
func (c *Conn) verifyServerCertificate(certificates [][]byte) error {
	...
	default:
		...
		return fmt.Errorf("tls: server's certificate contains an unsupported type of public key: %T", certs[0].PublicKey) //服务器的证书包含不受支持的公钥类型
	}
...
var (
	// 签名方案
	tls11SignatureSchemes      = []SignatureScheme{ECDSAWithP256AndSHA256, ECDSAWithP384AndSHA384, ECDSAWithP521AndSHA512, PKCS1WithSHA256, PKCS1WithSHA384, PKCS1WithSHA512, PKCS1WithSHA1, SM2WithSM3}
	tls11SignatureSchemesECDSA = tls11SignatureSchemes[:3]
	tls11SignatureSchemesRSA   = tls11SignatureSchemes[3:7]
	tls11SignatureSchemesSM2   = tls11SignatureSchemes[7:]
)
...
func certificateRequestInfoFromMsg(certReq *certificateRequestMsg) *CertificateRequestInfo {
	...
	if !certReq.hasSignatureAlgorithm {
		...
		// certificate types.
		switch {
		case rsaAvail && ecdsaAvail && sm2Avail:
		...
		case sm2Avail:
			cri.SignatureSchemes = tls11SignatureSchemesSM2   // 国密
		}
		return cri
	}
	...
	for _, sigScheme := range certReq.supportedSignatureAlgorithms {  // 支持的签名算法
		...
		case SignatureSM2:
			if sm2Avail {
				cri.SignatureSchemes = append(cri.SignatureSchemes, sigScheme) // 添加签名方案
			}
		}
	}
...
func (c *Conn) getClientCertificate(cri *CertificateRequestInfo) (*Certificate, error) {
	...
	for i, chain := range c.config.Certificates {
		sigOK := false
		for _, alg := range signatureSchemesForCertificate(c.vers, &chain) {  // 签名证书方案
