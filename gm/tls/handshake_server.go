...
func (c *Conn) readClientHello() (*clientHelloMsg, error) {
	...
	if c.config.GetConfigForClient != nil {
		...
		if newConfig, err := c.config.GetConfigForClient(chi); err != nil { //  获取客户端配置
...
func (hs *serverHandshakeState) pickCipherSuite() error {
	...
	if c.config.PreferServerCipherSuites { // 首选服务器密码套件
	...
	for _, id := range preferenceList {
		if hs.setCipherSuite(id, supportedList, c.vers) { //设置密码套件
			break
		}
	}
