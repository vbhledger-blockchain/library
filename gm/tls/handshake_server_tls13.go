...
func (hs *serverHandshakeStateTLS13) processClientHello() error {
	...
	if hs.suite == nil {
		c.sendAlert(alertHandshakeFailure)
		return errors.New("tls_v1.3: no cipher suite supported by both client and server")
	}
...
func (hs *serverHandshakeStateTLS13) sendServerCertificate() error {
	...
	if sigType == signatureRSAPSS {
		signOpts = &rsa.PSSOptions{SaltLength: rsa.PSSSaltLengthEqualsHash, Hash: crypto.Hash(sigHash)}
	}
...
func (hs *serverHandshakeStateTLS13) readClientCertificate() error {
	...
	if len(certMsg.certificate.Certificate) != 0 {
		...
		if sigType == signaturePKCS1v15 || crypto.Hash(sigHash) == crypto.SHA1 {
			c.sendAlert(alertIllegalParameter)
			return errors.New("tls: invalid certificate signature algorithm")
		}
		h := sigHash.New()
